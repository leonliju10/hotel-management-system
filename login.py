...
class register:
    def __init__(self,root):
        ...
        checkbtn = Checkbutton(frame,variable=self.var_check_btn,text="I AGREE THE TERMS AND CONDITIONS.",font=("arial",11,"bold"),onvalue=1,offvalue=0)
    ...
    def rgister_data(self):
        if self.var_fname.get()=="" or self.var_email.get()=="" or self.var_security_q.get()=="SELECT":
            ...
            if row!=None:
                ...
                my_cursor.execute("insert into register2 values (%s,%s,%s,%s,%s,%s,%s)",(self.var_fname.get(),self.var_lname.get(),self.var_contact.get(),self.var_email.get(),self.var_security_q.get(),self.var_security_a.get(),self.var_pass.get()))
                my_cursor.execute("insert into passcode values (%s,%s,%s,%s,aes_encrypt(%s,'key'),aes_encrypt(%s,'key'),aes_encrypt(%s,'key'))",(self.var_fname.get(),self.var_lname.get(),self.var_contact.get(),self.var_email.get(),self.var_security_q.get(),self.var_security_a.get(),self.var_pass.get()))
